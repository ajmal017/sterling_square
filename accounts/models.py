...
class UserDetails(models.Model):
    ...
    identity = models.ForeignKey(IdentityDetails, related_name='user_identity_rel', null=True, blank=True,
...
class StockInfo(models.Model):
    stock = models.OneToOneField(StockNames, related_name='stock_info', on_delete=models.SET_NULL, null=True,
                                 blank=True)
...
class StockGeneral(models.Model):
    Stockticker = models.OneToOneField(StockNames, related_name='general', on_delete=models.SET_NULL, null=True,
                                       blank=True)
...
class Transaction(models.Model):
    ...
    stockticker = models.ForeignKey(StockNames, related_name="stock_transaction_rel", on_delete=models.SET_NULL,
...
class Position(models.Model):
    ...
    stockname = models.ForeignKey(StockNames, related_name='stockposition_rel', on_delete=models.SET_NULL, null=True,
                                  blank=True)
    transaction_details = models.ForeignKey(Transaction, related_name='stockposition_transaction_rel',
    ...
    def __str__(self):
        return str(self.userid.first_name) + " >>> " + str(self.stockname.symbol) if self.userid else str(
...
class TopSearched(models.Model):
    userid = models.ForeignKey(User, related_name='user_top_search_rel', on_delete=models.SET_NULL, null=True,
                               blank=True)
    stock = models.ForeignKey(StockNames, related_name='stock_top_search_rel', on_delete=models.SET_NULL, null=True,
                              blank=True)
    count = models.IntegerField(max_length=10, default=0, null=True, blank=True)


class TransactionHistory(models.Model):
    position_obj = models.ForeignKey(Position, related_name='histroy_position_rel', on_delete=models.SET_NULL,
                                     null=True,
                                     blank=True)
