...
def get_new_history(symbol):
    ...
    past_3_months = today.date() - dateutil.relativedelta.relativedelta(months=18)
    sbin = tickerData.history(period='1d', start=past_3_months.strftime('%Y-%m-%d'),
        ...
        if str(stock.symbol) != "SYMBOL":
            ...
            try:
                ...
                try:
                    ...
                    pass
                try:
                    # print(str(stock.symbol)+".NS", "       priceToBook   ", yf_info["priceToBook"])
                ...
                try:
                    ...
                    try:
                        employees = get_earnings_by_type(ear_resp, "", "get_employees")
                        if employees:
                ...
                try:
                    ...
                    pass
                try:
                    about_company = get_earnings_by_type(ear_resp, "", "about_company")
                ...
                try:
                    headquaters = get_earnings_by_type(ear_resp, "", "get_headquaters")
                ...
                try:
                    pe_ratio = get_earnings_by_type(ear_resp, "", "get_pe_ratio")
                ...
                def limit_order_purchase(transaction_obj):
                    ...
                    if status_flag == 1:
                        ...
                        if transaction_obj.remove_date and transaction_obj.expires == "Good till canceled":
                            if current_date.date() == transaction_obj.remove_date.date():
...
def update_pos_table_latest_price():
    ...
    if flag == 1:
        for user_obj in User.objects.all():
            # current_cash = user_obj.user_details.all()[0].identity.buyingpower
            def update_position_table(user_obj):
                ...
                for pos_obj in user_obj.userposition_rel.all():
                    ...
                    try:
                        update_position_and_gainloss(live_price, pos_obj.stockname.symbol, user_obj)
                    except:
                        pass
                    try:
                        total_price += float(live_price) * float(pos_obj.transaction_details.size)
                    except:
                        pass
                date = datetime.datetime.timestamp(datetime.datetime.now())
